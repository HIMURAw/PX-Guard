const { guard_config: { logChannelWebHook, ban_limit }, emotes } = require("../../../config.js");
...
module.exports = async (member) => {
  ...
  await userModel
    .updateOne({ guildID: guild.id, userID: entry.executor.id }, { $inc: { "memberGuard.memberBan": 1 } }, { upsert: true })
  ...
  if (log) {
    const embed = new EmbedBuilder()
      ...
      .setDescription(`> ${entry.executor} yetkilisi **<t:${Math.floor(Date.now() / 1000)}:f>** tarihinde **kullanıcı banladı** ve gerekli işlemler uygulandı.`)
      .addFields([
        ...
        { name: "Yetkili Limiti ↷", value: "```" + String(`${Number(ban_limit.limit)}/${Number(userData?.memberBan || 0)}`) + "```", inline: true },
      ])
  ...
  if (Number(userData?.memberBan || 0) >= Number(ban_limit.limit)) {
    if (log) {
      const embed = new EmbedBuilder()
    ...
    await userModel
      .updateOne({ guildID: guild.id, userID: entry.executor.id }, { $set: { "memberGuard.memberBan": 0 } }, { upsert: true })
      .then(async () => await guildModel
        .updateOne({ guildID: guild.id }, { $push: { infos: { type: "Kullanıcı Yasaklama", userID: entry.executor.id, time: Date.now(), jail: true } } }, { upsert: true }))
  } else {
    await guildModel
      .updateOne({ guildID: guild.id }, { $push: { infos: { type: "Kullanıcı Yasaklama", userID: entry.executor.id, time: Date.now(), jail: false } } }, { upsert: true })
  }
};
module.exports.conf = {
  name: "guildBanAdd",
