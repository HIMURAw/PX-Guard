...
module.exports = async (emoji) => {
  ...
  const entry = await guild
    .fetchAuditLogs({ type: 62, limit: 1 })
  ...
  await userModel
    .updateOne({ guildID: guild.id, userID: entry.executor.id }, { $inc: { "emoteGuard.emoteDelete": 1 } }, { upsert: true })
  ...
  if (log) {
    const embed = new EmbedBuilder()
      ...
      .setDescription(`> ${entry.executor} yetkilisi **<t:${Math.floor(Date.now() / 1000)}:f>** tarihinde **emoji sildi** ve gerekli işlemler uygulandı.`)
      .addFields([
        ...
        { name: "Yetkili Limiti ↷", value: "```" + String(`${emote_limit.delete}/${userData?.emoteDelete}`) + "```", inline: true },
      ])
  ...
  if (Number(userData?.emoteDelete || 0) >= Number(emote_limit.delete)) {
    if (log) {
      const embed = new EmbedBuilder()
    ...
    await userModel
      .updateOne({ guildID: guild.id, userID: entry.executor.id }, { $set: { "emoteGuard.emoteDelete": 0 } }, { upsert: true })
      .then(async () => await guildModel
        .updateOne({ guildID: guild.id }, { $push: { infos: { type: "Emoji Silme", userID: entry.executor.id, time: Date.now(), jail: true } } }, { upsert: true }))
  } else {
    await guildModel
      .updateOne({ guildID: guild.id }, { $push: { infos: { type: "Emoji Silme", userID: entry.executor.id, time: Date.now(), jail: false } } }, { upsert: true })
  }
};
module.exports.conf = {
  name: "emojiDelete",
